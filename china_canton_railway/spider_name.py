spiders_name = {
        # 'china_canton_railway_spider' : '中国铁路广州局集团有限公司物资采购商务平台',     # 已爬
        # 'china_railway_luban_spider' : '中国中铁采购电子商务平台',
        # 'AirChina_spider' : '中国国际航空公司',
        # 'changjiang_waterway_spider' : '长江航道局',
        # 'yueyang_city_gov_spider' : '岳阳市人民政府',                    # 已爬
        # 'meishan_city_gov_spider' : '眉山市公共资源交易中心',                          ＃ 眉山爬虫较为特殊不修改
        # 'daqing_city_gov_spider' : '大庆市公共资源交易网',
        # 'zhuzhou_city_gov_spider' : '株洲市公共资源交易网',
        # 'longyan_city_gov_spider' : '眉山市公共资源交易网',              # 已爬
        # 'dalian_city_gov_spider' : '大连市公共资源',                         # 不通用所以不用改
        # 'foshan_nanhai_gov_spider' : '佛山市南海区人民政府网',           # 已爬
        # 'xinxiang_city_gov_spider' : '新乡市政府采购网',                  # 已爬
        # 'liupanshui_city_gov_spider' : '六盘水市公共资源交易网',           # js加密 已爬
        # 'jinzhou_city_gov_spider' : '锦州市公共资源交易管理办公室',         # 已爬
        # 'jingzhou_city_gov_spider' : '荆州市公共资源交易信息网',
        # 'anshun_city_gov_spider' : '安顺市全国公共资源交易平台',
        # 'hefei_city_gov_spider' : '合肥市政府信息公开网',                # done 写着写着502错误(已恢复)
        # 'dalizhou_city_gov_spider' : '大理州公共资源交易电子服务系统',     # 已爬
        # 'shaoxing_city_gov_spider' : '绍兴公共资源交易网',               # done
        # 'liuzhou_city_gov_spider' : '柳州市公共资源交易管理办公室',        # 已爬
        # 'yulin_city_gov_spider' : '玉林市人民政府门户网站',               # done
        # 'puer_city_gov_spider' : '普洱市公共资源交易电子服务系统',         # 已爬
        # 'yangzhou_city_gov_spider' : '扬州市政府采购网',                 # done
        # 'changchun_city_gov_spider' : '长春市公共资源交易网',             # done
        # 'jiaxing_city_gov_spider' : '嘉兴市公共资源交易中心',            # done
        # 'jinhua_city_gov_spider' : '金华市公共资源交易中心',      # 较为特殊内容是图片 # done
        # 'fuzhou_city_gov_spider' : '福州市公共资源交易网',               # done
        # 'tianjin_city_spider' : '天津市公共资源交易网',
        'tianjin_city_gov_spider' : '天津市政府采购网',
}